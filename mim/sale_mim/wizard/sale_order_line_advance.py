...
class SaleOrderLineaAdvance(models.TransientModel):
    ...
    select_type = fields.Many2one('product.product', 'Type', domain=[('categ_id.is_manufacture', '=', True)], change_default=True)
    ...
    vitre = fields.Many2one('mim.article', 'Vitre', domain=[('category_id', '=', 'Vitrage')])
    type_vitre = fields.Selection([('simple', 'Simple'), ('double', 'Double')], string="")
    decoratif = fields.Many2one('mim.article', 'Décoratif', domain=[('category_id', '=', 'Decoratif')])
    poigne = fields.Many2one('mim.article', 'Poignée', domain=[('category_id', '=', 'Poignee')])
    nb_poigne = fields.Integer('Nombre')
    serr = fields.Many2one('mim.article', 'Serrure', domain=[('category_id', '=', 'Serrure')])
